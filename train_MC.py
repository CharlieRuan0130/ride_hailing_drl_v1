...
# from utils import Logger, Scaler # Charlie commented out 5/3/22, utils not included, maybe just Scaler?
...
def run_policy(network_id, policy, scaler, logger, gamma,
    ...
    # weights = policy.sess.get_weights()  # get neural network parameters
    weights = policy.variables.get_weights() # Charlie 5/4/22, trying to make code run

    print('DEBUGGGGGGGGGGGGGGGG - DONE 0')

    ray.get([s.set_weights.remote(weights) for s in simulators]) # assign the neural network weights to all actors

    print('DEBUGGGGGGGGGGGGGGGG - DONE 1')
    ######################################################
    ...
    # commented out by Charlie 5/4/22 to make code run
    ...
    for j in range(actors_per_run):
        ...
        # Charlie 5/4/22                        
        accum_res.extend(ray.get([simulators[i].run_episode.remote(ray.get(network_id), ray.get(scaler_id)) for i in range(MAX_ACTORS)]))
    if remainder>0:
        # accum_res.extend(ray.get([simulators[i].run_episode.remote(network_id, scaler_id, time_steps, cycles_num,
        #                         skipping_steps, initial_states_set[actors_per_run*MAX_ACTORS+i]) for i in range(remainder)]))

        # Charlie 5/4/22
        accum_res.extend(ray.get([simulators[i].run_episode.remote(ray.get(network_id), ray.get(scaler_id)) for i in range(remainder)]))
    print('simulation is done')

    # commented by Charlie 5/4/22 to make code run
    ...
    for i in range(len(accum_res)):
        trajectories.append(accum_res[i])
    #################################################
...
def add_disc_sum_rew(trajectories, policy, network, gamma, lam, scaler, iteration):
    ...
    for trajectory in trajectories:
        ...
        if gamma < 1:
            ...
            # charlie 5/4/22 to make code run
            # disc_sum_rew = relarive_af(trajectory['unscaled_obs'], td_pi=trajectory['rewards'], lam=1)  # advantage function
            disc_sum_rew = relarive_af(trajectory['state'], td_pi=trajectory['reward'], lam=1)  # advantage function
    ...
    state_times = np.concatenate([t['state_time'][:-burn] for t in trajectories]) # charlie 5/4/22 to make code run
    ...
    if iteration ==1:
        for t in trajectories:
            # Charlie 5/4/22 to make code run
            # t['observes'] = (t['unscaled_obs'] - offset) * scale
            t['state_scaled'] = (np.array(t['state']) - offset) * scale # since list cannot take elemental operations

    return observes, disc_sum_rew_norm, state_times # charlie 5/4/22 to make code run
...
def build_train_set(trajectories, gamma, scaler):
    ...
    # unscaled_obs = np.concatenate([t['unscaled_obs'][:-burn] for t in trajectories])
    unscaled_obs = np.concatenate([t['state'][:-burn] for t in trajectories]) # Charlie 5/4/22 to make code run
    disc_sum_rew = np.concatenate([t['disc_sum_rew'][:-burn] for t in trajectories])

    scale, offset = scaler.get()
    # actions = np.concatenate([t['actions'][:-burn] for t in trajectories])
    actions = np.concatenate([t['action'][:-burn] for t in trajectories]) # Charlie 5/4/22 to make code run
    advantages = np.concatenate([t['advantages'][:-burn] for t in trajectories])
    # observes = (unscaled_obs - offset[:-1]) * scale[:-1]
    observes = (unscaled_obs - offset) * scale # Charlie 5/4/22 to make code run
...
def main(network_id, num_policy_iterations, gamma, lam, kl_targ, batch_size, hid1_mult, episode_duration,
    ...
    scaler = Scaler() # Charlie 5/3/22 to make code run
    cycles_num = None # Charlie 5/3/22 to make code run, dummy variable

    # Charlie 5/4/22 to make code run
    # val_func = NNValueFunction(obs_dim=400, hid1_mult=1, hid3_mult=5, sz_voc=360, embed_dim=6) # Value Neural Network initialization
    val_func = NNValueFunction(obs_dim=ray.get(network_id).obs_dim, hid1_mult=1, hid3_mult=5, sz_voc=360, embed_dim=6) # Value Neural Network initialization
    ...
    while iteration < num_policy_iterations:
        ...
        # add_value(trajectories, val_func, scaler)  # add estimated values to episodes ; commented by Charlie, probably not needed?
        observes, disc_sum_rew_norm, state_times = add_disc_sum_rew(trajectories, policy, ray.get(network_id), gamma, lam, scaler, iteration)  # calculate values from data

        # val_func.fit(observes, np.arange(1, ray.get(network_id).H), disc_sum_rew_norm, logger)  # update value function
        val_func.fit(observes, state_times, disc_sum_rew_norm, logger) # Charlie 5/4/22 to make code run
        ...
        policy.update(observes, state_times, actions, np.squeeze(advantages), logger)  # update policy
...
if __name__ == "__main__":
    ...
    with warnings.catch_warnings(): # Charlie 5/4/22 to supress all the deprecated warnings
