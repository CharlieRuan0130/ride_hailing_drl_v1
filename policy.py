...
class Policy(object):
    ...
    def set_weights(self, new_weights):
        """
        Charlie 5/3/22, added to make coed run; simply sets weights for the actor NN's
        """
    ...
    def _build_graph(self):
        ...
        with self.g.as_default():
            ...
            # self.variables = ray.experimental.tf_utils.TensorFlowVariables(self.loss, self.sess) # Charlie 5/3/22, version issues 
    ...
    def run_episode(self, network: Env, scaler):
        """
        ...
        while dec_epoch < network.H:
            ...
            while num_free_nby_cars > 0.5:  # cars' perspective
                ...
                while True:  # resample until feasible action
                    ...
                    # print('trip_type:',trip_type) # commented by Charlie 5/4/22 to make output clear
                    ...
                    if s_running[network.car_dim + trip_type - network.num_imp_ride[trip_type]] > 0.5:
                        # print('Car-passenger matching!')
                        rewards.append(network.c[slot_id][orig, dest])
                        # post-decision state
                        s_running[network.car_dim+trip_type-network.num_imp_ride[trip_type]] -= 1
                    ...
                    # post-decision state
                        ...
                        # Case 1) eta_w_car[0] > 0.5
                        # The candidate car has NOT reached its current destination yet
                        # Whatever empty-driving trip assigned now is too early; we will instead leave the car as it is
                        # with a new trip (not necessarily empty-driving anymore) assigned
                        # when it becomes closer to the destination in a later decision epoch

                        # Case 2) eta_w_car[0] <= 0.5 and dest==orig
                        # Action "staying empty at the destination":
                        # same total distance to the ASSIGNED NEXT destination, but not available anymore

                        # In both cases, cars are temporarily unavailable for the current decision epoch
        ...
        # Charlie 5/4/22 since queue has 0 elements sometimes
        matching_rate = -1
